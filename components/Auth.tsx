    ...
    <>
          ...
          <div className="p-16 flex flex-col bg-white dark:bg-dark-card overflow-y-auto max-h-screen transition-colors duration-500">
            <div className="mb-10">
              <div className="flex gap-2 p-1.5 bg-brand-surface dark:bg-dark-bg rounded-2xl mb-8">
                {(['student', 'teacher', 'admin'] as const).map((r) => (
                  <button
                    key={r}
                    type="button"
                    onClick={() => { setRole(r); setStep(1); setError(null); setNotification(null); }}
              ...
              <div className="flex justify-between items-end mb-2">
                ...
                {role !== 'admin' && (
                  <button 
                    onClick={() => { setIsLogin(!isLogin); setStep(1); setError(null); setNotification(null); }}
            ...
            {notification && (
              <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-sm font-bold rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-[10px] font-black text-brand-tertiary uppercase tracking-widest">Password</label>
                      <button
                        type="button"
                        onClick={() => { setShowForgotPassword(true); setError(null); setNotification(null); }}
                        className="text-xs font-bold text-brand-secondary dark:text-brand-tertiary hover:underline"
                      >
                        Forgot Password?
                      </button>
                    </div>
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-primary/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-dark-card w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-8 border-b border-brand-tertiary/10 flex items-center justify-between">
              <h3 className="text-xl font-black text-brand-primary dark:text-white">Reset Password</h3>
              <button onClick={() => { setShowForgotPassword(false); setError(null); }} className="text-brand-tertiary hover:text-rose-500 transition-colors"><i className="fa-solid fa-xmark text-xl"></i></button>
            </div>
            <form onSubmit={handlePasswordResetRequest} className="p-8 space-y-6">
              <p className="text-brand-tertiary font-medium">Enter your registered email address below. We'll send you a link to reset your password.</p>
              
              {error && (
                <div className="p-3 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-bold rounded-xl flex items-center gap-2">
                  <i className="fa-solid fa-circle-exclamation"></i>
                  {error}
                </div>
              )}
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => { setShowForgotPassword(false); setError(null); }} className="flex-1 py-4 font-black text-brand-tertiary rounded-2xl hover:bg-brand-surface transition-all">Cancel</button>
                <button type="submit" className="flex-[2] py-4 bg-brand-primary text-white rounded-2xl font-black shadow-xl transition-all active:scale-95">Send Reset Link</button>
              </div>
            </form>
          </div>
        </div>
